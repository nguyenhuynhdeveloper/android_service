...
public class MyService extends Service {
    ...
    // onStartCommand:  quyết định service bị kill : và sau khi bộ nhớ có đủ thì service sẽ ứng sử sao
    // Nơi nhận cái Intent mà ở StartService truyền tới
    @Override
    public int onStartCommand(Intent intent, int flags, int startId) {

   String strDataIntent = intent.getStringExtra("key_data_intent");   // Lây dữ liệu từ  intent

   // Đây chính là chạy notifycation ở trong Service
   sendNotification(strDataIntent);  // Chạy hàm gửi dữ liệu lên Notification
    ...
    private void sendNotification(String strDataIntent) {
        ...
        PendingIntent pendingIntent = PendingIntent.getActivity(
                ...
                0,
                intent,
                PendingIntent.FLAG_IMMUTABLE |PendingIntent.FLAG_UPDATE_CURRENT  //  Để có thể mở được màn hình MainActivity khi bấm vào notification
        );


        Notification notification = new NotificationCompat.Builder(this, CHANNEL_ID)   //  CHANNEL_ID được export ở bên _MyApplication
    ...
    // Vòng đời khi service bị destroy đi
    @Override
    public void onDestroy() {
        super.onDestroy();
        Log.e(TAG , "onDestroy: My Service onDestroy" );
    }
}
